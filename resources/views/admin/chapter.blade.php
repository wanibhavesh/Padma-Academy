                                ...
                                <div class="basic-form">
                                    <form method="post" action="{{route('chapter.store')}}">
                                        ...
                                        <div class="form-group">
                                            <input type="text" name="name" class="form-control input-default" placeholder="Chapter Name" required>
                                            
                                        </div>
                                        
                                        <div class="form-group">
                                            <input type="number" name="number" min="1" class="form-control input-default" placeholder="Chapter Number" required>
                                        ...
                                        <div class="form-group">
                                            <select class="form-control" name="code" required>
                                                ...
                                                @foreach($div as $row)
                                                <option value="{{$row['class_slug']}}">{{$row['class_name']}}</option>
                            ...
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered zero-configuration">
                                        ...
                                        <tbody>
                                           @foreach($chap as $rows)
                     
                                            <tr>
                                                <td>{{ $rows['chapter_id']}}</td>
                                                <td>{{ $rows['class_slug']}}</td>
                                                <td>{{ $rows['chapter_name']}}</td>
                                                <td>{{ $rows['chapter_number']}}</td>
                                                <td>{{ $rows['chapter_slug']}}</td>
                                                <td>
                                                    <a href="{{url('admin/editchapter',$rows['chapter_id'])}}" onclick=""><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                                                </td>
                                                <td>
                                                <form id="delete-form-{{ $rows['chapter_id']}}" method="post" action="{{ route('chapter.destroy',$rows['chapter_id'])}}" style="display:none">
                                                    {{csrf_field()}}
                                                    {{method_field('DELETE')}}
                                                </form>
                                                <a href="" onclick="
                                                if(confirm('Are you sure you want to delete?'))
                                                {
                                                    ...
                                                    document.getElementById('delete-form-{{$rows['chapter_id']}}').submit();
                                                }
                                                else{
                                                event.preventDefault();
